...
function DetailHistory(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      console.log(response.product_in_order);

      setCart(response.product_in_order);
  ...
  return (
    <div className="container">
      ...
      <div className="p-5">
        ...
        {information.user_id ? (
          <>
            <p>ID User: {information.user_id._id}</p>
            <p>Full Name: {information.user_id.fullname}</p>
            <p>Phone: {information.user_id.phone_number}</p>
            <p>Address: {information.user_id.address}</p>
      ...
      <div className="table-responsive pt-5 pb-5">
        <table className="table">
          ...
          <tbody>
            {cart &&
              cart.map((value) => (
                <tr className="text-center" key={value.product_id._id}>
                  <td className="align-middle border-0">
                    <h6 className="mb-0">{value.product_id._id}</h6>
                  </td>
                  ...
                  <td className="align-middle border-0">
                    <h6 className="mb-0">{value.product_id.name}</h6>
                  </td>
                  <td className="align-middle border-0">
                    <h6 className="mb-0">{value.product_id.price}</h6>
                  </td>
                  <td className="align-middle border-0">
